# 20/4 EN ESTE CODIGO VOY A REALIZAR LA SIMULACIÓN DE SILICIO CRISTALINO
...
file = './Files/Si_300micrometros_incoherente_300-900.pickle'
RT = RT_with_cache(file,stack_Si, materials, lams)
ref, trans = RT
# %%
js_Si_am0 = jmax_am0 - np.trapezoid(ref*am0*weight, dx=step).T
js_Si_am15 = jmax_am15 - np.trapezoid(ref*am15*weight, dx=step).T

T_js_Si_am0 =  np.trapezoid(trans*am0*weight, dx=step).T
T_js_Si_am15 =  np.trapezoid(trans*am15*weight, dx=step).T


#%%
# toda esta parte no sirve porque no hay variación de espesores
...
plt.plot(lams,ref,'o--',label="reflectancia AM0")
plt.plot(lams,ref,label="reflectancia AM1.5")
plt.grid()
plt.legend()
