...
savetxt = True#set True to save R,T and A to a txt file 
...
### simulation panel
        ...
        self.txt_wave_min = wx.TextCtrl(panel2,-1,"300")
        self.txt_wave_max = wx.TextCtrl(panel2,-1,"900")
...
#        mainsizer.Fit(nb)#adatto le dimensioni
    ...
    def on_btn_alpha(self, event):
        ...
        self.ema()
        self.checkrange()
        start=__builtins__.max(float(self.txt_wave_min.GetValue()),self.multilayer.lmin)
        end=__builtins__.min(float(self.txt_wave_max.GetValue()),self.multilayer.lmax)
        Fi=float(self.txt_angle.GetValue())
        Fi=pi*Fi/180.0
...
#            dlg.Destroy()
    ...
    def on_btn_compute(self,event):
        ...
        if end > start:
            ...
            if self.CheckBox_R.GetValue():
                self.axes.plot(lam, 100 * R)
                if savetxt:
                    # Lógica para guardar en TXT
                    import numpy as np
                    datos = np.column_stack((lam, R))
                    np.savetxt("reflectancia_gaas.txt", datos, header="Lambda(nm) R(%)")
